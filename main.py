...
def check_haarcascadefile():
    ...
    if exists:
        ...
        mess._show(title='Critical File Missing', message='Please Contact Us for Help')
        window.destroy()
...
def clicked(): 
    ...
    count = count + 1
    lbl.configure(text=f"Auto Generated ID is: {count}")
...
def psw():
    ...
    if (password == key):
        ...
        mess._show(title='Wrong Password', message='You have Entered a Wrong Password, Please try again!')
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + str(Id) + '.'+ ' ' + str(semester) + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
        ...
        res = "Images Taken for ID : " + str(Id)
        row = [serial, '', Id, '', name, '', semester]
        ...
        # make changes here to create restrictions on the ID button and method
        if (Id.isalpha() == True):
...
def TrainImages():
    ...
    try:
        ...
        mess._show(title='No Registrations', message='Please Register Some Students!')
        return
...
def TrackImages():
    ...
    if exists1:
        ...
        mess._show(title='Details Missing', message='Students Details are Missing, Make Sure the Files are Present in the Directory')
        cam.release()
        cv2.destroyAllWindows()
        window.destroy()
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                SEM = df.loc[df['SEMESTER'] == serial]['SEMESTER'].values
                ID = str(ID)
                ID = ID[1:-1]
                bb = str(aa)
                bb = bb[2:-2]
                attendance = [str(ID), '', bb, '', str(date), '','',str(SEM), str(timeStamp)]
    ...
    with open("Attendance\Attendance_" + date + ".csv", 'r') as csvFile1:
        ...
        for lines in reader1:
            ...
            if (i > 1):
                if (i % 2 != 0):
                    iidd = str(lines[0]) + '   '
                    tv.insert('', 0, text=iidd, values=(str(lines[2]), str(lines[4]), str(lines[6]),str(lines[7])))
    csvFile1.close()
    cam.release()
    cv2.destroyAllWindows()
...
clearButton = tk.Button(frame2, text="Generate ID", command=clicked  ,fg="black"  ,bg="#ff7221"  ,width=11 ,activebackground = "white" ,font=('comic', 11, ' bold '))
clearButton.place(x=335, y=86)

lbl = tk.Label(frame2, text="Click the Generate Button to Generate ID",width=37  ,height=1  ,fg="black"  ,bg="#c79cff" ,font=('comic', 13, ' bold ') )
lbl.place(x=80, y=55)

lbl = tk.Label(frame2, text=f"Auto Generated ID is: {count}",width=20  ,height=1  ,fg="black"  ,bg="#c79cff" ,font=('comic', 15, ' bold ') )
lbl.place(x=30, y=88)

# txt = tk.Entry(frame2, textvariable=count ,width=32 ,fg="black",font=('comic', 15, ' bold '))
# txt.place(x=30, y=88)
...
lbl3 = tk.Label(frame2, text="Enter Semester",width=20  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold '))
...
tv= ttk.Treeview(frame1,height =13,columns = ('name','date','time','semester'))
...
tv.heading('semester',text ='SEMESTER')

###################### SCROLLBAR ################################

scroll=ttk.Scrollbar(frame1,orient='horizontal',command=tv.xview)
scroll.grid(row=2,column=4,padx=(0,100),pady=(150,0),sticky='ns')
tv.configure(xscrollcommand=scroll.set)
...
clearButton2.place(x=335, y=162)
